...
export default function Search({ user }) {
  ...
  const fetchResults = async () => {
    ...
    const { data: sitesData } = await supabase
      .from('sites')
      .select('*')
      .or(`name.ilike.%${query}%,description.ilike.%${query}%,shortcuts.ilike.%${query}%`)
    ...
    // Generate featured snippet from top result
    if (sitesData && sitesData.length > 0) {
      const topSite = sitesData[0];
      setFeaturedSnippet({
        type: 'site',
        title: topSite.name,
        description: topSite.description,
        url: topSite.url,
        slug: topSite.slug
      });
      ...
      setFeaturedSnippet({
        type: 'wiki',
        title: topWiki.title,
        description: topWiki.content?.substring(0, 200) + '...',
        url: topWiki.url
      });
    } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#202124] text-gray-900 dark:text-gray-100 flex flex-col">
      ...
      <div className="bg-white dark:bg-[#303134] border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 py-4 shadow-sm">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center gap-4">
          <a href="/" className="flex items-center gap-3">
            <img src="/assets/logo.png" alt="Z&E Net" className="h-8 w-8" style={{ imageRendering: 'pixelated' }} />
            <span className="text-xl font-bold">Z&E <span className="text-blue-600">Net</span></span>
          </a>
          ...
          <form onSubmit={handleSearch} className="flex-grow w-full">
            <input
              ...
              placeholder="Search..."
              className="w-full px-4 py-2 bg-gray-100 dark:bg-[#202124] border border-gray-300 dark:border-gray-700 rounded-full focus:outline-none focus:border-blue-500"
            />
          </form>

          <div className="flex gap-3">
            <button onClick={toggleTheme} className="text-sm text-gray-600 dark:text-gray-400">
              {isDark ? '☀️' : '🌙'}
            </button>
            {user ? (
              <a href="/account" className="text-sm text-gray-600 dark:text-gray-400">Account</a>
            ) : (
              <a href="/login" className="text-sm text-gray-600 dark:text-gray-400">Sign in</a>
            )}
          </div>
        </div>
      </div>
      ...
      <main className="flex-grow max-w-4xl mx-auto px-4 sm:px-6 py-6 w-full">
        {/* Featured Snippet - Google AI Style */}
        {featuredSnippet && !loading && (
          <div className="mb-6 bg-white dark:bg-[#303134] border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-sm">
            <div className="flex items-start gap-4">
              <div className="flex-grow">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  ...
                  {featuredSnippet.url && (
                    <>
                      <span>•</span>
                      <a href={featuredSnippet.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline truncate max-w-md">
                        {featuredSnippet.url}
                      </a>
                    </>
                  )}
              ...
              {featuredSnippet.slug && (
                <button
                  onClick={() => navigate(`/site/${featuredSnippet.slug}`)}
          ...
          <div className="space-y-4">
            {siteResults.map((site) => (
              <div
                ...
                className="bg-white dark:bg-[#303134] border border-gray-200 dark:border-gray-700 rounded-xl p-5 hover:shadow-md transition-shadow cursor-pointer"
              >
                <div className="flex items-start gap-3">
                  <div className="flex-grow min-w-0">
                    <h3 className="text-xl font-semibold text-blue-600 dark:text-blue-400 mb-1 truncate">
                      {site.name}
                    </h3>
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-500">
                      ...
                      {site.shortcuts && site.shortcuts.length > 0 && (
                        <span className="text-gray-400">Shortcuts: {site.shortcuts.join(', ')}</span>
                      )}
                      <span>•</span>
                ...
                <div className="space-y-3">
                  {wikiResults.map((page) => (
                    <a
                      ...
                      className="block bg-white dark:bg-[#303134] border border-gray-200 dark:border-gray-700 rounded-xl p-4 hover:shadow-md transition-shadow"
                    >
