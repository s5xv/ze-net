  ...
  const fetchData = async (tab) => {
    ...
    try {
      ...
      if (tab === 'withdrawals') {
        ...
        const [sitesRes, regsRes] = await Promise.all([
          supabase.from('sites').select('*').order('created_at', { ascending: false }),
          supabase.from('business_registrations').select('*').order('created_at', { ascending: false })
        ]);
        setBusinessRegistrations([...(sitesRes.data || []), ...(regsRes.data || [])]);
            ...
            {businessRegistrations.length === 0 ? (
              ...
              <div className="space-y-3">
                {businessRegistrations.map(b => {
                  const isSite = !!b.slug;
                  return (
                  <div key={b.id + (isSite ? '-site' : '-reg')} className="bg-[#303134] border border-gray-700 rounded-xl p-4">
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <p className="text-white font-bold text-lg">{isSite ? b.name : b.business_name}</p>
                          {isSite ? (
                            <a href={`/site/${b.slug}`} className="text-blue-400 text-xs hover:underline">View</a>
                          ) : (
                            <span className="text-xs text-yellow-400">Registration</span>
                          )}
                        </div>
                        <p className="text-sm text-gray-400">
                          {isSite ? b.category + (b.subcategory ? ' / ' + b.subcategory : '') : b.category}
                          {b.plot_number ? ` &middot; Plot #${b.plot_number}` : ''}
                        </p>
                        ...
                        {isSite && b.is_verified && <p className="text-xs text-blue-400 mt-1">✓ Verified</p>}
                        {isSite && <p className="text-xs text-gray-500 mt-1">Views: {b.view_count || 0}</p>}
                        {!isSite && b.owner_discord && <p className="text-xs text-gray-500 mt-1">Discord: {b.owner_discord}</p>}
                        {!isSite && b.website_url && <p className="text-xs text-gray-500 mt-1">URL: {b.website_url}</p>}
