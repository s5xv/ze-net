  ...
  if (action === 'summarize') {
    ...
    try {
      ...
      const prompt = hasResults
        ? `You are the 'Z&E Net AI Search Assistant', an expert navigation tool for the DemocracyCraft Minecraft server directory.\n\nUser Query: "${query}"\n\nDatabase Results:\n${resultsText}\n\nInstructions:\n1. STRICT RELEVANCE: Analyze the query. Discard results where the query word only appears as a substring in an unrelated word.\n2. COMPREHENSIVE LISTING: If the user is searching for a category (like "departments", "shops", "banks"), you MUST list EVERY single item found in the database results below. Do not skip any. Do not summarize the list.\n3. SYNTHESIS: Write a highly engaging summary. Start directly with the answer. Mention the specific names of the best matching sites, wiki pages, or departments.\n4. FORMATTING: Use bold text (markdown) for the names of the sites/departments to make them pop.\n5. ZERO HALLUCINATION: ONLY use the provided database results. Never invent information.\n\nOutput ONLY the final summary text. Do not include any introductory phrases.`
        : `You are a helpful assistant for the DemocracyCraft Minecraft server. You have a good general knowledge of DemocracyCraft, including its government departments (Education, Interior, DCT, Commerce, State, Justice, etc.), common server features, and how the server works. Use your knowledge to answer the user's question Helpfully.\n\nCRITICAL: NEVER mention specific games, minigames, shops, sites, plugins, or features unless you are 100% certain they exist on DemocracyCraft. If unsure, say "I don't have specific information about that in the directory."\n\nGeneral guidelines about DemocracyCraft you CAN mention:\n- The server is a towny/economy/political roleplay server\n- It has departments like Education, Interior, DCT, Commerce, State, Justice, etc.\n- Players can own businesses, join political parties, vote in elections\n- There is an economy with a treasury, shops, and banks\n\nUser Question: "${query}"\n\nKeep the answer concise (1-3 paragraphs). Use markdown bold for key terms. At the end, if you used general knowledge rather than directory data, add: "\n\n*Based on general knowledge — check the directory for verified listings*"`;
